...
def plot_results(config):
    ...
    for model in models:
        ...
        for qps in qps_values:
            #result_path = Path("results/benchmark") / dataset_name / model_name / policy / f"qps_{qps}"
            ...
            for file in result_path.glob("*.json"):
                ...
                if throughput is not None:
                    data_by_dataset[dataset_name][model_name].append((qps, latency_ms))
    ...
    for dataset, model_data in data_by_dataset.items():
        ...
        for i, (model, results) in enumerate(model_data.items()):
            ...
            plt.bar(positions, latency_ms, bar_width, label=model_rename.get(model, model))

        plt.xlabel("Sessions")
        plt.ylabel("Request Latency P99 (ms)")
